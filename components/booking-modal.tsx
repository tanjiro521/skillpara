...
export function BookingModal({ provider, currentUser, isOpen, onClose, onSuccess }: BookingModalProps) {
  ...
  const [selectedMethod, setSelectedMethod] = useState<"money" | "tokens" | "swap" | null>(null)
  ...
  // Removed automatic mode selection to force user choice
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isSkillSwap = selectedMethod === "swap"
    const isCreditBooking = selectedMethod === "tokens"
          ...
          <form onSubmit={handleSubmit}>
                ...
                <div>
                  ...
                  <div className="mt-2 grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div
                      ...
                      className={`p-4 rounded-xl border cursor-pointer transition ${selectedMethod === "money"
                          ? "border-purple-600 bg-purple-50 shadow-md dark:bg-purple-900/20"
                          : "border-gray-200 hover:border-purple-300 dark:border-gray-700"
                        }`}
                    >
                      <div className="font-medium text-lg mb-1">💰 Money (Online / Offline)</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400">Pay directly for the session</div>
                    </div>

                    <div
                      onClick={() => setSelectedMethod("tokens")}
                      className={`p-4 rounded-xl border cursor-pointer transition ${selectedMethod === "tokens"
                          ? "border-purple-600 bg-purple-50 shadow-md dark:bg-purple-900/20"
                          : "border-gray-200 hover:border-purple-300 dark:border-gray-700"
                        }`}
                    >
                      <div className="font-medium text-lg mb-1">🪙 Tokens</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400">Use earned credits to learn</div>
                    </div>

                    <div
                      onClick={() => {
                        setSelectedMethod("swap")
                        setShowSkillSwapFlow(true)
                      }}
                      className={`p-4 rounded-xl border cursor-pointer transition ${selectedMethod === "swap"
                          ? "border-purple-600 bg-purple-50 shadow-md dark:bg-purple-900/20"
                          : "border-gray-200 hover:border-purple-300 dark:border-gray-700"
                        }`}
                    >
                      <div className="font-medium text-lg mb-1">🔄 Skill Swap</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400">Exchange your skill instead of paying</div>
                    </div>
                  </div>
                  <p className="mt-2 text-sm text-purple-600 dark:text-purple-400 italic">
                    No money? Choose Skill Swap or Tokens
                  </p>
                </div>

                {selectedMethod === "swap" && currentUser.skills?.length > 0 && (
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {currentUser.skills
                        ...
                        .map((skill: Skill) => (
                          <div
                            key={skill.id}
                            className={`p-3 border rounded-md cursor-pointer ${selectedSeekerSkill === skill.id
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={!selectedSlot || !selectedMethod || isLoading}
              >
